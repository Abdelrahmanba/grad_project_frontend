...
export default function Sem({ semId }) {
  const { kid } = useParams()
  ...
  const fetchSemster = async (semId) => {
    const res = await get(`/semesters/${semId}?includeKindergarten=false`, token)
    if (res.ok) {
      const resJson = await res.json()
      setSemster(resJson)
    } else {
  ...
  const fetchKindergarten = async () => {
    ...
    if (res.ok) {
      ...
      if (resJson.runningSemester != null) {
        await fetchSemster(semId)

        await fetchAllA(1, semId)
        await fetchAllChildren(1, semId)
        setLoading(false)
      }
    }
    setLoading(false)
  ...
  const fetchAllChildren = async (page = 1, semId) => {
    setLoading(true)
    setPage(page)
  ...
  if (loading) {
    ...
    return (
      <div>
        ...
        <div>
          <h2 style={{ marginTop: 0 }}>All Enrolled Children for this Semester</h2>
          <Table
            bordered
            size='large'
            pagination={{
